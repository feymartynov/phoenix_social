defmodule PhoenixSocial.PostChannelSpec do
  ...
  let :user, do: insert(:user) |> Repo.preload(:friendships)
  let :user_socket, do: socket("users:#{user.id}", %{current_user: user})

  context "with feed" do
    let :post, do: insert(:post, user: user, author: user)
    let :comment, do: insert(:comment, post: post)
  ...
  context "with wall" do
    let :wall_user, do: insert(:user)
    let :post, do: insert(:post, user: wall_user)
    let :comment, do: insert(:comment, post: post)

    before do
      socket |> subscribe_and_join(PostChannel, "wall:#{wall_user.id}")
    end
