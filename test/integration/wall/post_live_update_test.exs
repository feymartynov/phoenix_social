defmodule PhoenixSocial.Integration.Wall.PostLiveUpdateTest do
  ...
  test "Add a post", %{user: user, friend: friend} do
    # the user signs in, goes to the friend's profile and waits for updates
    ...
    # the user should see the friend's post on the wall without page refresh
    in_browser_session :user_session, fn ->
  ...
  test "Delete a post", %{user: user, friend: friend} do
    ...
    # the post should disappear from the wall
    in_browser_session :user_session, fn ->
      assert {:error, _} = search_element(:css, "#wall li[data-post-id]", 1)
    end
